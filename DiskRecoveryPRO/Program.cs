...
public class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        var logoBox = new PictureBox {
            Left = 18, Top = 10, Width = 62, Height = 62,
        ...
        var title = new Label {
            Text = "DISK RECOVERY PRO",
            Left = 92, Top = 14, Width = 600, Height = 34,
            Font = new Font("Segoe UI", 18, FontStyle.Bold),
            ForeColor = Color.FromArgb(48, 50, 65)
        };
        header.Controls.Add(title);

        var sub = new Label {
            Text = "Profesjonalne odzyskiwanie danych",
            Left = 94, Top = 47, Width = 500, Height = 20,
            ForeColor = Color.FromArgb(105, 108, 125)
        };
        ...
        licenseStatus.Click += (_, _) => MessageBox.Show(
            LicenseManager.IsUnlimited()
                ? "Licencja OWNER/PRO: pełny dostęp i brak limitu odzyskiwania."
                : "Licencja darmowa: 2 operacje odzyskiwania z dysku w miesiącu.\n\nPozostało: " + LicenseManager.GetRemainingThisMonth(),
            "Licencja", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        var scanButton = new Button {
            Text = "Skanuj dysk",
            Left = 20, Top = 20, Width = 180, Height = 44
        };
        scanButton.Click += (_, _) => Log("Skanowanie dysku gotowe do uruchomienia w pełnej wersji.");
        body.Controls.Add(scanButton);

        var info = new Label {
            Text = "Tryb: tylko utracone/niewidoczne pliki.\nOdzysk zapisuje dane wyłącznie na wybranym dysku docelowym.",
            Left = 20, Top = 84, Width = 700, Height = 55
        };
    ...
    async Task CheckForUpdatesAsync()
    {
        ...
        try
        {
            ...
            System.Diagnostics.Process.Start(new ProcessStartInfo { FileName = "cmd.exe", Arguments = "/c \"" + updaterBat + "\"", UseShellExecute = false, CreateNoWindow = true, WorkingDirectory = tempRoot });
            Close();
        }
        catch (Exception ex)
        {
